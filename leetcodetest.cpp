...
// Recursive function to compute operations
int recursiveDivide(int number1, int multiplier1, int number2, int multiplier2, int X) {
    // Base case: stop if both numbers are less than X
    if (number1 < X && number2 < X) 
        return 0;
    
    int operations = 0;

    // First number's breakdown
    if (number1 >= X) {
        int first1 = number1 / 2; // Floor division
        int second1 = (number1 + 1) / 2; // Ceil division
        operations += multiplier1 * 2; // Each part counts as two operations
        operations += recursiveDivide(first1, multiplier1 * 2, second1, multiplier1 * 2, X);
    }

    // Second number's breakdown
    if (number2 >= X) {
        int first2 = number2 / 2; // Floor division
        int second2 = (number2 + 1) / 2; // Ceil division
        operations += multiplier2 * 2; // Each part counts as two operations
        operations += recursiveDivide(first2, multiplier2 * 2, second2, multiplier2 * 2, X);
    }
...
int32_t main() {
    int N = 98; // Starting number
    int X = 47; // Threshold for stopping
    int multiplier = 1; // Initial multiplier is 1
    
    int operations = recursiveDivide(N / 2, multiplier, (N + 1) / 2, multiplier, X);
    std::cout << "Total operations: " << operations << endl;
