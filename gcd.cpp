...
int findFloorN(int k) {
    double a = 1, b = 1, c = -2 * k;
    double discriminant = sqrt(b * b - 4 * a * c);
    double n = (-b + discriminant) / (2 * a);
    int floor_n = static_cast<int>(n);
    if (floor_n * (floor_n + 1) / 2 > k) {
        floor_n--;
    }
    return floor_n;
}
...
void solve() {
    ...
    while (k > 0) {
        ...
        for(int b = 0;b<morerem;b++){
            v[i] = primes[b]*primes[b+1];
        }
    }
    if (k == 0) {
        for (int i = 0; i < n; i++) {
...
int32_t main() {
    ...
    cin.tie(nullptr);
        vector<int> revprimes = primes;
    reverse(revprimes.begin(), revprimes.end());
