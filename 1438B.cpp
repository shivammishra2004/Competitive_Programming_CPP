...
int32_t main(){
    ...
    while(t--){
        ...
        for(int i=0;i<n;i++){
            mpp1[v[i]]--;
            if(mpp1[v[i]] ==0) mpp1.erase(v[i]);
            mpp2[v[i]]++;
            map<int,int> mpp3{mpp1.begin(),mpp1.end()};
            map<int,int> mpp4{mpp2.begin(),mpp2.end()};
    std::map<int, int> temp;

    for (const auto& i : mpp3) {
        temp[i.first + 1] += (i.second / 2);
    }
    debug(mpp3);
    debug(temp);

    // Apply changes from the temporary map to the original map
    for (const auto& i : temp) {
        mpp3[i.first] += i.second;
    }
    debug(mpp3);
    debug(temp);
    // Erase original keys
