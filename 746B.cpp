...
int32_t main(){
    ...
    for(int i=0;i<n;i++){
        if(n&1) (i&1)?v.push_front(s[i]): v.push_back(s[i]);
        else (i&1)?v.push_back(s[i]):v.push_front(s[i]);
        // debug(pos);
    }
    ...
    // int n;string s; cin >> n >>s;
    // string s1 = s;
    // vector<char> v(n);int pos = 0;\
    // int a = (n-1)/2;
    // debug(a);
    // for(int i=0;i<n;i++){
    //     if(n&1) pos = ((i&1)?(a-ceil(i/2.0)):(a+ceil(i/2.0)));
    //     else pos = ((i&1)?(a+ceil(i/2.0)):(a-ceil(i/2.0)));
    //     debug(pos);
    //     v[pos] = s[i];
    // }
    // debug(v);
