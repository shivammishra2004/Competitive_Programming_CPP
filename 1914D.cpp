...
int32_t main(){
    ...
    while (t--) {
        ...
        for (int i = 0; i < n; ++i) {
            int maxi = max({a[i], b[i], c[i]});

            // Insert `maxi` into the deque while maintaining descending order
            auto it = dq.rbegin(); // Start from the back (reverse iterator)
            while (it != dq.rend() && *it < maxi) {
                ++it;
            }
            dq.insert(it.base(), maxi);

            // Keep only the largest 3 values
            if (dq.size() > 3) {
                dq.pop_back();
            }
        }
        int ans = 0;
        // Output the contents of the deque
