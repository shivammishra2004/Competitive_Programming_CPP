...
int32_t main(){
    ...
    fl(i,n)cin >> v1[i];
    srt(v1);
    fl(i,n){sum += v1[i]; v2[i] = sum;}
    int high = v1.back(), low = v1[0];
    int mid; 
    bool flag = true;
    // debug(v1);
    // debug(v2);
    while(low<=high){
        mid = low + (high-low)/2;
        int index = distance(v1.begin(),lower_bound(v1.begin(),v1.end(), mid))-1;
        // myFunc(mid,index);
        int y = v2.back() - v2[index];
        int size = n-index-1;
        // myFunc(mid,index,y-size*mid);
        // cerr << endl;
        if(m>y-size*mid) high = mid-1;
        else if(m<y-size*mid) low = mid+1;
