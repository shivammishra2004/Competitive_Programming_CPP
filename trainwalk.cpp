...
int32_t main() {
    ...
    while (--) {
        int num_cities, start, end, train_start, train_end, walk_cost, train_cost, max_wait;
        cin >> num_cities >> start >> end >> train_start >> train_end >> walk_cost >> train_cost >> max_wait;

        int positions[num_cities + 1];
        for (int i = 1; i <= num_cities; i++) {
            cin >> positions[i];
        }

        int walk_distance = walk_cost * abs(positions[start] - positions[end]);

        bool train_available = max_wait >= walk_cost * abs(positions[start] - positions[train_start]);
        int train_distance = max_wait + train_cost * abs(positions[train_start] - positions[train_end]) 
                             + walk_cost * abs(positions[train_end] - positions[end]);

        int min_cost = walk_distance;
        if (train_available) {
            min_cost = min(min_cost, train_distance);
        }

        cout << min_cost << NEWLINE;
    }
