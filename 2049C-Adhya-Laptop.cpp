...
void solve(int t) {
   ...
   int left_neighbor = (y - 2 + n) % n;
   int right_neighbor = y % n;
   if (n % 3 == 1){
       v[n-1] = 1;
       v[n-2] = 0;
   }
   
   vector<int> temp = {v[x - 1], v[left_neighbor], v[right_neighbor]};
   v[y - 1] = mex(temp);
   
   temp = {v[y - 1], v[(right_neighbor + 1) % n]};
   v[right_neighbor] = mex(temp);
   
   temp = {v[(left_neighbor - 1 + n) % n], v[y - 1]};
   v[left_neighbor] = mex(temp);

   v[y-1] = mex({v[x-1],v[(y-2+n)%n],v[y%n]});
