...
void solve() {
    ...
    // Iterate through the heightMap, ignoring the boundary elements
    for (int i = 1; i < n - 1; ++i) {
        for (int j = 1; j < m - 1; ++j) {
            // Find the max elements in the 4 directions
            
            // Right: from (i, j) to the end of the row
            int a = *max_element(heightMap[i].begin() + j + 1, heightMap[i].end());
            
            // Left: from (i, j) to the beginning of the row
            int b = *max_element(heightMap[i].begin(), heightMap[i].begin() + j);
            
            // Up: from (i, j) to the top of the column
            int c = heightMap[i][j];
            for (int k = i - 1; k >= 0; --k) {
                c = max(c, heightMap[k][j]);
            }
            
            // Down: from (i, j) to the bottom of the column
            int d = heightMap[i][j];
            for (int k = i + 1; k < n; ++k) {
                d = max(d, heightMap[k][j]);
            }
            
            // Find the min of the 4 maximum values
            int min_val = min({a, b, c, d});
            
            myFunc(a,b,c,d,min_val);
            // Add to the sum
            if(min_val > heightMap[i][j]) sum += min_val-heightMap[i][j];
        }
    }
