...
int recur(vector<int>& v, int sum) {
    if (sum == 0) return 0;
    // if (sum < 0) return 1e9;
    ...
    for (int i = 0; i < v.size(); i++) {
        if(sum>=v[i])mini = min(mini, recur(v, sum-v[i]));
    }
    return dp[sum] = mini+1;
    // if(mini == INT_MAX) return dp[sum] = mini;
    // else return dp[sum] = mini+1;
}

int iter(vector<int> & v,int sum){

    for(int i =1;i<=sum;i++){
        int mini = 1e9;
        for(int j = 0;j<v.size();j++){
            if(i>=v[j]) mini = min(mini,dp[i-v[j]]);
        }
        dp[i] = mini+1;
    }
    return dp[sum];
...
void solve() {
    ...
    // int result = iter(v, sum);
    int result = recur(v,sum);
