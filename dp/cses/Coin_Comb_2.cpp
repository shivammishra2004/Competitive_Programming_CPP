...
// int recur(int sum,int idx,vector<int>& nums){
//     if(sum == 0) return 1;
//     if(idx == nums.size()) return 0;
//     if(dp[sum][idx] != -1) return dp[sum][idx];
//     int val =0;
//     if(sum >= nums[idx]){
//         val = (val + recur(sum - nums[idx], idx, nums)) % MOD;
//         val = (val + recur(sum, idx + 1, nums)) % MOD;
//     }
//     return dp[sum][idx] = val;
// }

int iter(int sum,vector<int>& nums){
    for(int i=0;i<n;i++) dp[i][0] = 1;
    for(int i=1;i<=sum;i++) dp[n-1][sum] = 0;
...
void solve() {
    ...
    // cout << recur(sum,0,coins) << endl;

    dp.resize(sum+1,vector<int>(n,0));
    cout << recur(sum,coins) << endl;
}
